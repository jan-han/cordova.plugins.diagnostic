        ...
        cordova.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    java.lang.reflect.Method showSplashMethod = splashScreenClass.getDeclaredMethod("showSplashScreen", boolean.class);
                    showSplashMethod.setAccessible(true);
                    showSplashMethod.invoke(this, true);  // true 表示自动隐藏

                    instance.cordova.getActivity().recreate();

                    // 手动显示启动页
                     SplashScreen.show(cordova.getActivity());
                } catch (Exception ex) {
